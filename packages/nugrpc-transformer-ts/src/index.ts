...
export default class TransformerTS extends TransformAdapter implements Saveable {
  ...
  constructor (context: TransformContext) {
    ...
    this.writer.write(' */')
    this.writer.write("import { $axios, ApiResponse } from '@privyid/nugrpc-api'")
    this.writer.line()

    this.writer.write(`const $api = $axios.create({ baseURL: '${context.baseUrl ?? '/'}' })`)
    this.writer.line()
  }
  ...
  transformService (service: Service) {
    ...
    for (const method of service.methodsArray) {
      ...
      this.writer.tab(1)
      this.writer.write(`return $api.post('${url}', body)`)
      this.writer.tab(-1)
      this.writer.write('},')
    }
